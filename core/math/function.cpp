...
Math::Function::Function(QString* expr, Math::plotterParameters* params, bool first) {
    ...
    for(i=0; i<5; i++) {
        ...
        if (num >= 1) {
            if (i == 2) {
                parts = Support::divide(c[i], expression, num);
                while (!(parts[1]->length() > 0 && verify_str_par(parts[0])) && j > 0) {
                    j--;
                    parts = Support::divide(c[i], expression, j);
                }
            }
            else {
                parts = Support::divide(c[i], expression, 1);
...
double Math::Function::Compute(double x, double parameter) {
    switch (type) {
        ...
        case 'a': //Constant
            return value;
        case 'b': //Sinc
            return Support::sinc(connectedNodes[0]->Compute(x, parameter));
        case 'c': //x
            return x;
        case 'd': //Rect
            return Support::Rect(connectedNodes[0]->Compute(x, parameter));
        case 'e': //Delta
            return Support::delta(connectedNodes[0]->Compute(x, parameter), params->getWidth(), params->getXAxis());
        case 'f': //Abs
            return abs(connectedNodes[0]->Compute(x, parameter));
        case 'g': //Tri
            return Support::Tri(value, connectedNodes[0]->Compute(x, parameter));
        case 'h': //Parameter
            return parameter;
        default: return 0;
    }
